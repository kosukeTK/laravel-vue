<template>
  <div class="">
    ...
    <section class="todoapp">
      ...
      <div class="header">
        <h1>todos</h1>
        <input class="new-todo" autofocus autocomplete="off" placeholder="するべきことは？" v-model="state.newTodo" @keyup.enter="addTodo()" />
        <h2>{{ state.newTodo }}</h2>
      </div>
      <!-- todoList section -->
      <section class="main" v-show="state.todos.length" v-cloak>
        ...
        <!-- todoList -->
        <ul class="todo-list">
          <li v-for="todo in state.todos" class="todo" :key="todo.id" :class="{ completed: todo.is_complete, editing: todo == state.editedTodo }">
            <div class="view">
              <!-- 完了フラグ切替 -->
              <input class="toggle" type="checkbox" v-model="todo.is_complete" @change="activeTodoNow(todo.id, todo.is_complete)" />
              <!-- todo編集 -->
              <label @dblclick="editTodo(todo)">{{ todo.name }}</label>
              <!-- todo削除 -->
              <button class="destroy" @click="deleteTodo(todo.id)"></button>
            </div>
            <input class="edit" type="text" v-model="state.name" @blur="doneEdit(todo.id)" @keyup.enter="doneEdit(todo.id)" @keyup.esc="cancelEdit(todo.id)" />
          </li>
        </ul>
      </section>
