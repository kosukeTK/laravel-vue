<template>
    <div class="container">
        <div class="drop_area" accept="image/*"
            :class="{enter:state.isEnter}"
            @dragenter="dragEnter()"
            @dragleave="dragLeave()"
            @dragover.prevent
            ...
            <label for="selectFile">
                ...
                <input type="file" id="selectFile" accept="image/*" @change="selectFile($event)"
    ...
    export default{
        ...
        setup() {
            ...
            const fileProcessing = async (file, index) => {
                try{
                    let compFile = await ImageUtil.getCompressImageFileAsync(file)
            ...
            //  form.append('file', state.files)
                ...
                axios.post('albumDetail/upload', form, config)
                .then(res => {
